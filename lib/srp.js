...
var utilsSha1 = require('./utils/sha1');
...
function SrpClient(opts) {
	...
	var srp_data = {
		...
		radixb: opts.radix || 64, // 10 or 16 or 64
		protob: opts.proto || '6a', // 3 or 6 or 6a
		params: opts.params || params_values['1024'],
		srp_N: '',
		srp_g: '',
		srp_k: '',
		// Should transit to server only once
		srp_username: opts.username || '',
		srp_password: opts.client === true ? '' : opts.password,  
		// Should be generated by server
		srp_salt: opts.salt || '', // Will be random of not given
		srp_v: '',
		// Client
		srp_a: opts.a || '',
		srp_A: opts.A || '',
		srp_password2: opts.server === true ? '' : opts.password,  
		srp_x2: '',
		srp_Sc: '',
		// Server
		srp_B: opts.B || '',
	...
	function calcSHA1(data) {
		return utilsSha1.calcSHA1(data);
	}

	function calcSHA1Hex(data) {
		return utilsSha1.calcSHA1Hex(data);
	}
	...
	function get_random_n() {
	    var n = utilsBigInt.randomBigInt(32);
	    if (n.compareTo(N) >= 0) {
	        n = n.mod(N.subtract(one));
	    }
	...
	function srp_initialize() {
	    ...
	    if (!srp_data.srp_salt) {
	        console.warn("Warning: 'slat' option (Shared Secret) not provided random value generated");
	        set_random_salt();
	    } else {
	        isync_salt();
	    }

	    if (!srp_data.srp_a && opts.server !== true) {
	        console.warn("Warning: 'a' option (Client seed) not provided random value generated");
	        set_random_a();
	    } else if (opts.server !== true) {
	        isync_a();
	    }

	    if (!srp_data.srp_b && opts.client !== true) {
	        console.warn("Warning: 'b' option (Server seed) not provided random value generated");
	        set_random_b();
	    } else if (opts.client !== true) {
	        isync_b();
	    }
	...
	// TODO create API that allow server and client Ss and Sc exchange
	return Object.create(null, {
		...
		checkToken: {
			value: function (token) {
				...
				if (opts.client) {
					result = srp_data.srp_Sc === token;
				} else if (opts.server) {
					result = srp_data.srp_Ss === token;
				} else {
					result = srp_data.srp_Ss === token && srp_data.srp_Sc == srp_data.srp_Ss;
				}
		...
		checkPassword: {
			value: function (password) {
				...
				return srp_data.srp_Sc == srp_data.srp_Ss
			}
		}
	});
}
